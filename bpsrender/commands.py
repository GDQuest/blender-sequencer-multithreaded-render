...
def get_commands_video(cfg, clargs, **kwargs):
    ...
    out = map(lambda w: (w, kwargs['frame_start'] + w * chunk_length),
              range(clargs.workers))
    out = map(lambda x: (x[1],
                         x[1] + chunk_length - 1
                         if x[0] != clargs.workers - 1 else
                         kwargs['frame_end']), out)
    out = map(lambda x: get_commands(cfg, clargs, 'chunk',
                                     w_frame_start=x[0],
                                     w_frame_end=x[1],
                                     **kwargs), out)
...
def get_commands(cfg, clargs, what='', **kwargs):
    ...
    return (islice(get_commands_f, 2, None)
            if what == '' else
            (what, get_commands_f[what](cfg, clargs, **kwargs)))
...
def get_commands_all(cfg, clargs, **kwargs):
    ...
    out = (map(lambda k: k[0], out)
           if all(map(lambda k: not k[1], out)) else
           map(lambda k: k[0], filter(lambda k: k[1], out)))
